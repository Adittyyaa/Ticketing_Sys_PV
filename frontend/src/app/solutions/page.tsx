...
import { authService, adminService, solutionService } from '@/services'
...
export default function SolutionsPage() {
  ...
  const fetchSolutions = useCallback(async () => {
    try {
      ...
      const response = await solutionService.getSolutions(
        searchQuery.trim() ? { search: searchQuery.trim() } : selectedCategory ? { category: selectedCategory } : undefined
      )
      const result = response.data as any

      if (!response.success) throw new Error(result?.error || 'Failed to load solutions')

      setSolutions(result?.solutions || result?.data || [])
    } catch {
  ...
  const handleDelete = (e: React.MouseEvent, solution: Solution) => {
    ...
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          const response = await solutionService.deleteSolution(solution.id)
          if (!response.success) throw new Error(response.error || 'Failed to delete solution')
  ...
  const handleSubmit = async (values: any) => {
    ...
    try {
      ...
      let response;
      if (editingSolution) {
        response = await solutionService.updateSolution(editingSolution.id, payload)
      } else {
        response = await solutionService.createSolution(payload)
      }
      const result = response.data as any

      if (!response.success) throw new Error(result?.error || 'Failed to save solution')
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                      ...
                      <div>
                        ...
                        <div style={{ marginTop: 4 }}>
                          <Tag color="blue" style={{ borderRadius: 4, fontSize: 11 }}>{solution.category_name || (solution as any).category || 'General'}</Tag>

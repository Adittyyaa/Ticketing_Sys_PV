...
import { AppShell, TicketTable, TicketCardView, TicketInboxView, TicketFilterDrawer } from '@/components'
...
function filterTickets(
  ...
  return tickets.filter((ticket) => {
    ...
    const searchableText = `${ticket.title} ${ticket.description} ${ticket.number} ${ticket.category_name || ''} ${ticket.type_name || ''} ${ticket.product || ''} ${ticket.product_reference_number || ''} ${ticket.assigned_to_name || ''} ${ticket.user_email || ''}`.toLowerCase()
  ...
  useEffect(() => {
    ...
    const fetchTickets = async () => {
      ...
      try {
        ...
        if (!response.success) {
          if (response.error) {
            setTicketError(response.error)
          }
          return
        }

        fetchedTickets = (result?.tickets || result?.data || []) as Ticket[]
    ...
    const fetchStatuses = async () => {
      try {
        ...
        if (response.success && response.data) {
          setCustomStatuses(response.data)
        }
      } catch { /* silent */ }
    }
    fetchStatuses()

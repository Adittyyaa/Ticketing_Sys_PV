...
import { ArrowLeft, Download, Edit2, Trash2, CheckCircle } from 'lucide-react'
import Link from 'next/link'
import { useAuthStore } from '@/lib/store'
import { AppShell, CommentsSection as TicketComments, AttachmentsSection } from '@/components'
...
const statusOptions: CustomStatus['name'][] = ['UNTOUCHED', 'PENDING', 'OPENED', 'SOLVED']
...
export default function TicketDetailPage() {
    ...
    const checkAuth = async () => {
      try {
        ...
        if (adminResponse.success && adminResponse.data) {
          ...
          userData = { email: authResponse.data.user.email, full_name: authResponse.data.user.full_name, role: authResponse.data.user.role }
        }
  ...
  useEffect(() => {
    ...
    const fetchTicket = async () => {
      try {
        const response = await adminService.getTicketById(ticketId)
  ...
  useEffect(() => {
    ...
    const fetchStatuses = async () => {
      try {
        const response = await adminService.getCustomStatuses()
        if (response.success && response.data) {
          setCustomStatuses(response.data as CustomStatus[])
        }
      } catch { /* silent */ }
    }
    fetchStatuses()
  ...
  const handleSave = async (values: { priority: string; status: string }) => {
    if (!ticket) return
    try {
      const response = await adminService.updateTicket(ticketId, { priority: values.priority, status: values.status })
      if (!response.success) throw new Error('Failed to update ticket')
    ...
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          ...
          if (!response.success) throw new Error('Failed to delete ticket')
  ...
  const updateStatus = async (newStatus: string, msg: string) => {
    if (!ticket) return
    try {
      const response = await adminService.updateTicket(ticketId, { status: newStatus })
  ...
  const p = priorityDisplay[ticket.priority as keyof typeof priorityDisplay] || priorityDisplay.MEDIUM

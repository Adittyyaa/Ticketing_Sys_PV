...
export const useAuthStore = create((set, get) => ({
  ...
  logout: () => {
    ...
    // Clear the in-memory auth header (async lazy import avoids SSR issues)
    import('../services/api').then(({ apiClient }) => apiClient.clearAuthToken()).catch(() => {})
  },
  ...
  initializeAuth: () => {
    ...
    if (token) {
      ...
      import('../services/api').then(({ apiClient }) => apiClient.setAuthToken(token)).catch(() => {})
    } else {
      set({ loading: false })
    }
  },

...
async function setupDatabase() {
  ...
  if (!DATABASE_URL) {
    ...
    console.log('Set it like: postgresql://user:pass@host:port/db')
    process.exit(1)
  }
  ...
  try {
    ...
    const setupScript = await readFile(
      join(__dirname, '..', 'config', 'database-setup-postgresql.sql'),
      'utf8'
    )
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  if (args.includes('--create-admin')) {
    ...
    if (!email || !password) {
      ...
      console.log('Usage: --create-admin --email <email> --password <password> [--name <name>]')
      process.exit(1)
    }
